...
@Service
@Log4j2
public class OrchestratorTransactionMappingService implements IOrchestratorTransactionMappingService {
    private final SagaTransactionalJsonHelper sagaTransactionalJsonHelper;

    public OrchestratorTransactionMappingService(SagaTransactionalJsonHelper sagaTransactionalJsonHelper) {
        this.sagaTransactionalJsonHelper = sagaTransactionalJsonHelper;
    }

    @Override
    public Optional<OrchestratorTransactionMapping> findGlobalMapping() {
        return sagaTransactionalJsonHelper.loadTransactionsMappings()
            .stream()
    ...
    @Override
    public Optional<OrchestratorTransactionMapping> findGlobalMappingByBankCode(String bankCode) {
        return sagaTransactionalJsonHelper.loadTransactionsMappings()
            .stream()
            .filter(p -> StringUtils.isBlank(p.getServiceId()))
            .filter(p -> StringUtils.isBlank(p.getMessageTypeIndicator()))
    ...
    @Override
    public Optional<OrchestratorTransactionMapping> findTransactionMapping(String serviceId, String messageTypeIndicator, String processingCode) {
        return sagaTransactionalJsonHelper.loadTransactionsMappings()
            .stream()
    ...
    @Override
    public Optional<OrchestratorTransactionMapping> findTransactionMappingByBankCode(String serviceId, String messageTypeIndicator, String processingCode, String bankCode) {
        return sagaTransactionalJsonHelper.loadTransactionsMappings()
            .stream()
            .filter(p -> StringUtils.trimToEmpty(serviceId).equals(p.getServiceId()))
            .filter(p -> StringUtils.trimToEmpty(messageTypeIndicator).equals(p.getMessageTypeIndicator()))
